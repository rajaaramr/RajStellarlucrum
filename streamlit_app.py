...
async def get_worksheet():
    try:
        ...
        spreadsheet = await client.open("RajTask7_OptionData")  # Your sheet name
        worksheet = await spreadsheet.worksheet("Sheet1")       # Tab name
        return worksheet
    except Exception as e:
        st.error(f"⚠️ Error accessing Google Sheets:\n\n{e}")
        return None

# === Dummy Option Data Fetcher ===
...
if st.button("📥 Fetch Option Data"):
    ...
    try:
        ...
        worksheet = loop.run_until_complete(get_worksheet())
        if worksheet:
